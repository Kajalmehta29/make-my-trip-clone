  ...
  const confirmCancelBooking = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8081/booking/cancel?userId=${user?.id}&bookingId=${selectedBookingId}&cancellationReason=${cancellationReason}`,
        {
          method: "POST",
        }
      );
      ...
      if (response.ok) {
        // Update the user's bookings in the Redux store
        const updatedBookings = user?.bookings.map((booking: any) =>
          booking.bookingId === selectedBookingId
            ? { ...booking, cancelled: true, cancellationReason: cancellationReason }
            : booking
        );
